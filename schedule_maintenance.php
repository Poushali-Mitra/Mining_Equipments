<?php
if($_SERVER["REQUEST_METHOD"] == "POST") { //use because when we click the submitt buttoon by filling up the details tb hi code chlega niche kaphp    
...
<div class="form-container">
    ...
    <?php if (isset($_GET['success'])): //if url comming success?>
        <div class="success">Maintenance scheduled successfully.</div>
    <?php endif;//jab bhi succesfully submit ho jayega to url success hoga tb echo hoga ?>
    ...
    <form method="POST" action="schedule_maintenance.php">
        ...
        <select name="equipment_id" required>
            ...
            if ($equipment_result->num_rows > 0) {//Check if there are any rows (equipment) returned by the SQL query. num rows tell how many records
                while ($row = $equipment_result->fetch_assoc()) {//fetch_assoc() gets one row from the result as an associative array. means by key not by index
